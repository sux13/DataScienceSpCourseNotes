...
* in the above graph, the elements are as follows:
	- $red$ lines = means for two groups ($t = 0$ vs $t = 1$) --> two lines representing `lm(y ~ t)`
	- $black$ lines = regression lines for two groups ($t = 0$ vs $t = 1$) --> two lines representing `lm(y ~ t + x)`
	- $blue$ line = overall regression of $y$ vs $x$ --> line representing `lm(y ~ x)`
...
* [**odds**](http://en.wikipedia.org/wiki/Odds_ratio) are useful in constructing logistic regression models and fairly easy to interpret
    ...
    - $$E[earnings]= X p - Y (1 - p) = 0 \Rightarrow \frac{Y}{X} = \frac{p}{1 - p}$$
...
(move~\log(NH_i)~to~right~side)~\log\left(E[NHSS_i | JD_i, \beta0, \beta1]\right) & = \log(NH_i) + \beta0 + \beta1 JD_i \\
\end{aligned}$$
