...
### Quantile
* the $\alpha^{th}$ quantile of a distribution with distribution function F = point x$_{\alpha}$ 
	* F(x$_{\alpha}$) =  $\alpha$
...
* if we divide the about quantities over each other (same denominator), we get the following $$\frac{P(D\:|\:+)}{P(D^c\:|\:+)} = \frac{P(+\:|\:D)}{P(+\:|\:D^c)} \times \frac{P(D)}{P(D^c)}$$
* **odds** = $p/(1-p)$
	* $\frac{P(D)}{P(D^c)}$ = odds of disease in absence of test
	* $\frac{P(D\:|\:+)}{P(+\:|\:D^c)}$ = odds of disease given a positive test result
* **Diagnostic Likelihood Ratio** of a positive test result is defined as $$DLR_+ = \frac{P(+\:|\:D)}{P(Dc\:|\:+)}$$
...
* **average of random variables** = a new random variable where its distribution has an expected value that is the **same** as the original distribution (centers are the same) 
	...
	* if $E[sample~mean]$ = population mean, then estimator for the sample mean is **unbiased**
		...
		* since $E(aX) = aE(X)$, the expected value of the mean is can be written as $$E[\frac{X_1 + X_2 + X_3 + ... + X_n}{n}] = \frac{1}{n} \times [E(X_1) + E(X_2) + E(X_3) + ... + E(X_n)]$$
...
E[\frac{X_1 + X_2 + X_3 + ... + X_n}{n}] = \frac{1}{n} \times [E(X_1) + E(X_2) + E(X_3) + ... + E(X_n)]\\
...
* dividing by $n - 1$ will make the variance estimator ***larger*** to adjust for this fact --> leads to more accurate estimation --> $S^2$ = so called ***unbiased estimate of population variance*** 
...
* **binomial random variable** = sum of **n** Bernoulli variables = $\sum X_i$ where $X_i$ = $Bernoulli(p)$
	* PMF is defined as $$P(X=x) = {n \choose x}p^x(1-p)^{n-x}$$ where ${n \choose x}$ = number of ways selecting $x$ items out of $n$ options without replacement or regard to order
	* the **combination** or "$n$ choose $x$" is defined as $${n \choose x} = \frac{n!}{x!(n-x)!}$$
...
* `pbinom(6, size=8, prob =0.5, lower.tail=TRUE)` = probability of 6 or less successes out of 8 samples with probability of 0.5
...
* $X \sim N(0, 1)$ = **standard normal distribution** (standard normal random variables often denoted using $Z_1, Z_2, \ldots$)
	* ~68% of data/normal density --> between $\pm$ 1 standard deviation from $\mu$
	* ~95% of data/normal density --> between $\pm$ 2 standard deviation from $\mu$
	* ~99% of data/normal density --> between $\pm$ 3 standard deviation from $\mu$
	* $\pm$ 1.28 standard deviations from $\mu$ --> 10$^{th}$ (-) and 90$^{th}$ (+) percentiles
	* $\pm$ 1.645 standard deviations from $\mu$ --> 5$^{th}$ (-) and 95$^{th}$ (+) percentiles
	* $\pm$ 1.96 standard deviations from $\mu$ --> 2.5$^{th}$ (-) and 97.5$^{th}$ (+) percentiles
	* $\pm$ 2.33 standard deviations from $\mu$ --> 1$^{st}$ (-) and 99$^{th}$ (+) percentiles
...
* distribution of means of IID variables approaches the standard normal as $n$ increases
...
* ***Note**: does not guarantee that the normal distribution will be a good approximation, but just that eventually it will be a good approximation as n --> $\infty$ *
...
* sample mean is a  
* **95% confidence interval for the population mean $\mu$** is defined as $$\bar X \pm 2\sigma^2/n$$ for the sample mean $\bar X \sim N(\mu, \sigma^2/n)$
	* you can choose to use 1.96 to be more accurate for the confidence interval 
	* $P(\bar{X} > \mu + 2\sigma^2/n~or~\bar{X} < \mu - 2\sigma^2/n = 5%$
...
### Wald Confidence Interval (Bernoulli Distribution)
* for Bernoulli distributions, confidence interval takes the form
    $$\hat{p} \pm z_{1-\alpha/2}\sqrt{\frac{p(1-p)}{n}}$$
* $p$ = unknown, so use $\hat{p} = X/n$ to replace
* $p(1-p)$ is largest when $p = 1/2$, so confidence interval becomes $$\hat{p} \pm \frac{1}{\sqrt{n}}$$
...
* `binom.test(k, n)` = returns confidence interval for `k` successes in `n` draws

### Agresti/Coull Interval (Binomial Distribution)
* for a binomial distribution with smaller values of $n$ (when n is not large enough for CLT) we can construct the **Agresti/Coull Interval**, which is defined as $$\hat{p} = \frac{X+2}{n+4}$$ 
* this is effectively ***adding 2*** to number of successes, $X$ and ***adding 2*** to number of failure
* interval tend to be ***conservative***
...
* Hypothesis testing = making decisions using data
    ...
    * assumed to be ***true*** --> statistical evidence required to reject it for **alternative** or "research" hypothesis (**H$_a$**)
...
* ***family wise error rate (FWER)*** = probabilit of at least one false positive $Pr(V \ge 1)$
...
* as we can see from the black line, the observed difference/statistic is very far from the mean --> likely 0 is ***not*** the true difference
