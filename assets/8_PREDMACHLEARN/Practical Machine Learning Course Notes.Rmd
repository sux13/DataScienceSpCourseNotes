...
* **components of predictor** = question --> input data --> features _(extracting variables/characteristics)_ --> algorithm --> parameters _(estimate)_ --> evaluation
...
```{r fig.width = 6, fig.height = 4, fig.align = 'center', message = FALSE, cache = TRUE}
# load iris data set
data(iris)
...
```{r fig.width = 5, fig.height = 4, fig.align = 'center', message = FALSE, warning = FALSE, cache = TRUE}
# load data
...
```{r fig.width = 5, fig.height = 3, fig.align = 'center', message = FALSE, warning = FALSE, cache = TRUE}
# load data
data(iris)
...
* if we substitute $f_k(x)$ and $f_l(x)$ with Gaussian probability density functions $$f(x) = \frac{1}{\sigma \sqrt{2 \pi}}e^{-\frac{(x-\mu)^2}{2\sigma^2}}$$ so the ratio can be simplified to $$\log \frac{P(Y = k~|~X=x)}{P(Y = j~|~X=x)} = \underbracket[0.5pt]{\log \frac{\pi_k}{\pi_j}}_{\text{constant}} - \underbracket[0.5pt][6pt]{\frac{1}{2}(\mu_k + \mu_j)^T \Sigma^{-1}(\mu_k + \mu_j)}_{\text{constant}} + x^T \underbracket[0.5pt][10pt]{\Sigma^{-1} (\mu_k - \mu_j)}_{\text{slope}}$$ where $\Sigma^{-1}$ = covariance matrix for the predictor variables, $x^T$ = set of predictor variables, and $\mu_k$ / $\mu_j$ = mean of $k$, $j$ respectively 
...
```{r fig.width = 5, fig.height = 4, fig.align = 'center', cache = TRUE}
...
```{r fig.width = 5, fig.height = 4.5, fig.align = 'center', message = FALSE, warning = FALSE, cache = TRUE}
