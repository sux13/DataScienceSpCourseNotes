...
* best fit line with estimated slope and intercept ($X$ as predictor, $Y$ as outcome) --> $$Y = \hat \beta_0 + \hat \beta_1 X$$ where $$\hat \beta_1 = Cor(Y, X) \frac{Sd(Y)}{Sd(X)} ~~~ \hat \beta_0 = \bar Y - \hat \beta_1 \bar X$$
...
* goal is use statistics to draw inferences --> generalize from data to population through models
...
* it can then be said to have $Y_i$ as independent $N(\mu, \sigma^2)$, where $\mu = \beta_0 + \beta_1 x_i$ <-- likelihood equivalent model
	- **likelihood** = given the outcome, what is the probability?
		...
		+ above is the **probability density function ** of $n$ samples from the normal distribution --> this is because the regression line is normally distributed due to $\epsilon_{i}$
...
$$
	...
	- ***example***:
		...
		+ converting $X$ to $cm$ $\Longrightarrow$ multiplying $X$ by $100 cm/m$
		+ this mean $\beta_1$ has to be divided by $100 cm/m$ for the correct units.
$$
...
* `diamond` dataset from `UsingR` package
	- diamond prices in Singapore Dollars, diamond weight in carats (standard measure of diamond mass, 0.2$g$)
...
* to find the **maximum likelihood estimator (MLE)** of the mean, $\mu$, we take the derivative of the likelihood $\mathcal{L}$ with respect to $\mu$ --> $\frac{\partial \mathcal{L}}{\partial \mu}$
...
* $e_i$ can also be interpreted as the outcome ($Y$) with the linear association of the predictor ($X$) removed
	- or, "Y adjusted for X"
* $E[e_i] = 0$ --> this is because the mean of the residuals is expected to be 0 (assumed Gaussian distribution)
	- the Gaussian distribution assumption also implies that the error is **NOT** correlated with any predictors
	- `mean(fitModel$residuals)` = returns mean of residuals --> should equal to 0
	- `cov(fit$residuals, predictors)` = returns the covariance (measures correlation) of residuals and predictors --> should also equal to 0
...
* however, a more common approach is to use $$\hat \sigma^2 = \frac{1}{n-2}\sum_{i=1}^n e_i^2$$
	- $n-2$ is used instead of $n$ to make the estimator ***unbiased*** --> $E[\hat \sigma^2] = \sigma^2$
...
* $R^2$ = sample correlation squared
	- `cor(outcome, predictor` = calculates the correlation between predictor and outcome --> the same as calculating $R^2$
* $R^2$ can be a ***misleading*** summary of model fit
	- deleting data --> inflate $R^2$.
...
\end{aligned}
$$
* therefore, interpretation of a multivariate regression coefficient --> expected change in the response per unit change in the regressor, holding all of the other regressors fixed
...
```{r fig.height = 3.5, fig.width=8, message = F, warning = F, fig.align = 'center'}
# simulate data
...
```
* as we can see above, the R dropped the unnecessary variable `z` by excluding it from the linear regression --> `z`'s coefficient is `NA`
...
* ***Note**: we are also assuming the error terms $\epsilon_{Fi}$ and $\epsilon_{Mi}$ are Gaussian distributions --> mean = 0 * 
...
* the following code simulates the linear model, $$Y_i = \beta_0 + \beta_1 x_i + \beta_2 t_i + \epsilon_i$$ where $t = {0, 1}$ --> binary variable
...
* in the above graph, the elements are as follows:
	- $\textcolor{red}{red}$ lines = means for two groups ($t = 0$ vs $t = 1$) --> two lines representing `lm(y ~ t)`
	- $black$ lines = regression lines for two groups ($t = 0$ vs $t = 1$) --> two lines representing `lm(y ~ t + x)`
	- $\textcolor{blue}{blue}$ line = overall regression of $y$ vs $x$ --> line representing `lm(y ~ x)`
...
* the following code simulates the linear model, $$Y_i = \beta_0 + \beta_1 x_i + \beta_2 t_i + \beta_3 x_i t_i + \epsilon_i$$ where $t = {0, 1} --> binary variable
...
* in the above graph, the elements are as follows:
	- $\textcolor{red}{red}$ lines = means for two groups ($t = 0$ vs $t = 1$) --> two lines representing `lm(y ~ t)`
	- $black$ lines = regression lines for two groups ($t = 0$ vs $t = 1$) --> two lines representing `lm(y ~ t + x + t*x)`
	- $\textcolor{blue}{blue}$ line = overall regression of $y$ vs $x$ --> line representing `lm(y ~ x)`
...
* from the generated plots above, we can see that 
	...
	- relationship between $x_{1}$ and $y$ (loosely correlated  --> $R^2$ = `r round(summary(lm(y ~ x1))$r.squared,2)`) ***largely unchanged*** by when $x_{2}$ is considered
...
* variable selection simply affects associations between outcome and predictors, using the model to formulate causal relationship are even more difficult (entire field dedicated to this --> ***causal inference***) 
...
### General Rules
* **omitting variables** --> generally results in ***increased bias*** in coefficients of interest
...
* **including irrelevant/unnecessary variables** --> generally ***increases standard errors*** (estimated standard deviation) of the coefficients
...
* whenever highly correlated variables are included in the same model --> the standard error and therefore the***variance*** of the model ***increases*** --> this is known as **variance inflation**
...
* assuming that the model is linear with additive iid errors (with finite variance), we can mathematically describe the impact of omitting necessary variables or including unnecessary ones
	- **underfitting** the model --> variance estimate is ***biased*** --> $E[\hat \sigma^2] \neq \sigma^2$
	- **correctly fitting** or **overfitting** the model --> variance estimate is ***unbiased*** --> $E[\hat \sigma^2] = \sigma^2$
		+ however, if unnecessary variables are included, the variance estimate is ***larger*** than that of the correctly fitted variables --> $Var(\hat \sigma_{overfitted}) \geq Var(\hat \sigma_{correct})$
...
* limitations of linear models:
	- response can be discrete (i.e. 0, 1, etc.) or strictly positive --> linear response models don't make much sense
...
* **general linear models** = introduced in 1972 RSSB paper by Nelder and Wedderburn and has ***3*** parts
	...
	2. systematic component for linear predictor --> incorporates the information about the independent variables into the model
...
* [**odds**](http://en.wikipedia.org/wiki/Odds_ratio) are useful in constructing logistic regression models and fairly easy to interpret
    ...
    - $$E[earnings]= X p - Y (1 - p) = 0 \Rightarrow \underbracket[0.5pt]{\frac{Y}{X}}_{\text{payout ratio}} = \underbracket[0.5pt]{\frac{p}{1 - p}}_{\text{odds}}$$
	+ odds can be interpretted as "How much should you be willing to pay for a $p$ probability of winning a dollar?"
...
* the traffic can be modeled using linear model as follows $$ NH_i = \beta0 + \beta1 JD_i + \epsilon_i $$
	- $NH_i$ = number of hits to the website
	- $JD_i$ = day of the year (Julian day)
	- $\beta0$ = number of hits on Julian day 0 (1970-01-01)
	- $\beta1$ = increase in number of hits per unit day
	- $\epsilon_i$ = variation due to everything we didn't measure
* the expected outcome is defined as $$ E[NH_i | JD_i, \beta0, \beta1] = \beta0 + \beta1 JD_i$$
...
### Example - log Outcome
- if we are interested in relative increases in web traffic, we can the natural log of the outcome, so the linear model becomes $$ \log(NH_i) = \beta0 + \beta1 JD_i + \epsilon_i$$
	- $\log(NH_i)$ = number of hits to the website
	- $JD_i$ = day of the year (Julian day)
	- $\beta0$ = log number of hits on Julian day 0 (1970-01-01)
	- $\beta1$ = increase in log number of hits per unit day
	- $\epsilon_i$ = variation due to everything we didn't measure
...
* the expected outcome is $$E[\log(NH_i | JD_i, \beta0, \beta1)] = \beta0 + \beta1 JD_i $$  
...
- the Poisson model can be constructed as log of the mean $$\log\left(E[NH_i | JD_i, \beta0, \beta1]\right) = \beta0 + \beta1 JD_i$$ or in other form  $$E[NH_i | JD_i, \beta0, \beta1] = \exp\left(\beta0 + \beta1 JD_i\right)$$
	- $NH_i$ = number of hits to the website
	- $JD_i$ = day of the year (Julian day)
	- $\beta0$ = expected number of hits on Julian day 0 (1970-01-01)
	- $\beta1$ = expected increase in number of hits per unit day
...
* we can transform the Poisson model to $$E[NH_i | JD_i, \beta0, \beta1] = \exp\left(\beta0 + \beta1 JD_i\right) = \exp\left(\beta0 \right)\exp\left(\beta1 JD_i\right)$$
	- $\beta_1 = E[NH_i | JD_i+1, \beta0, \beta1] - E[NH_i | JD_i, \beta0, \beta1]$ 
...
E[NHSS_i | JD_i, \beta0, \beta1]/NH_i & = \exp\left(\beta0 + \beta1 JD_i\right) \\
(take~\log~of~both~sides)~\log\left(E[NHSS_i | JD_i, \beta0, \beta1]\right) - \log(NH_i) & = \beta0 + \beta1 JD_i \\
(move~\log(NH_i)~to~right~side)~\log\left(E[NHSS_i | JD_i, \beta0, \beta1]\right) & = \underbracket[0.5pt]{\log(NH_i)}_{\text{offset}} + \beta0 + \beta1 JD_i \\
\end{aligned}$$
...
E[NHSS_i | JD_i, \beta0, \beta1]/24 & = \exp\left(\beta0 + \beta1 JD_i\right) \\
(take~\log~of~both~sides)~\log\left(E[NHSS_i | JD_i, \beta0, \beta1]\right) - \log(24) & = \beta0 + \beta1 JD_i \\
(move~\log(24)~to~right~side)~\log\left(E[NHSS_i | JD_i, \beta0, \beta1]\right) & = \log(24) + \log(NH_i) + \beta0 + \beta1 JD_i \\
\end{aligned}$$
...
(simplify)~ E[Y_i]_{\xi = 5 | right} & = \underbracket[0.5pt]{\beta_0 - 5 \gamma_k}_{\text{intercept}} + \underbracket[0.5pt]{(\beta_1 + \gamma_k)}_{\text{slope}} x_i \\
\end{aligned}$$
